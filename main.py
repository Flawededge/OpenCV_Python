...
class MainPlotGui(Ui_OpenCVThresholder):
    ...
    def load_image(self):  # Loads image from a filename
        ...
        self.disp_image(cvtColor(self.inputImage, COLOR_BGR2RGB))  # Display the image on the left side
        self.process_image()  # Process the image, which displays it on the right

    def disp_image(self, image):  # Shows the given image on 0 (input) or 1 (output)
